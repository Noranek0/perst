...
public class CompressedReadWriteFile implements IFile {
     ...
     *                                     after reaching this size page index is
     *                                     flushed and log is truncsated
     * @param pageIndexInitSize            initial size of page index
     * @param readOnly                     whether access to the file is read-only
     * @param noFlush                      whether synchronous write to the disk
     *                                     should be performed
     * @param cipherKey                    cipher key (if null, then no encryption
     *                                     is performed)
     */
    public CompressedReadWriteFile(String dataFilePath,
        ...
        try {
            ...
            if (!readOnly) {
                ...
                while ((size -= 8) >= 0) {
                    ...
                    long pageBitSize = ((pagePos & (Page.pageSize - 1))
                            + ALLOCATION_QUANTUM) >>> ALLOCATION_QUANTUM_LOG;
                    BitmapAllocator.reserve(bitmap, pageBitOffs, pageBitSize);
                }
            }
        } catch (IOException x) {
            throw new StorageError(StorageError.FILE_ACCESS_ERROR, x);
    ...
    long allocate(int bitSize) {
        long pos = BitmapAllocator.allocate(bitmap, bitmapPos, bitmap.length, bitSize);
        if (pos < 0) {
            pos = BitmapAllocator.allocate(bitmap, bitmapStart, BitmapAllocator.locateHoleEnd(bitmap, bitmapPos),
                    bitSize);
            if (pos < 0) {
                byte[] newBitmap = new byte[bitmap.length + bitmapExtensionQuantum];
                System.arraycopy(bitmap, 0, newBitmap, 0, bitmap.length);
                pos = BitmapAllocator.allocate(newBitmap, BitmapAllocator.locateBitmapEnd(newBitmap, bitmap.length),
